...
class CarbonCalculator:
    ...
    def _generate_recommendations(self, data: CompanyData, breakdown: Dict[str, float]) -> List[str]:
        ...
        for source, emissions in sorted_sources[:3]:  # Top 3 sources
            percentage = (emissions / total_emissions) * 100
            
            if source == 'electricite' and percentage > 20:
                recommendations.append(
                    f"⚡ L'électricité représente {percentage:.1f}% de vos émissions. "
                    "Considérez un contrat d'électricité verte ou l'installation de panneaux solaires."
                )
            
            elif source == 'achats' and percentage > 30:
                recommendations.append(
                    f"🛒 Les achats représentent {percentage:.1f}% de vos émissions. "
                    "Privilégiez les fournisseurs locaux et éco-responsables."
                )
            
            elif source == 'vols_internationaux' and percentage > 15:
                recommendations.append(
                    f"✈️ Les vols internationaux représentent {percentage:.1f}% de vos émissions. "
                    "Favorisez la visioconférence et compensez vos vols."
                )
            
            elif source == 'vehicules' and percentage > 25:
                recommendations.append(
                    f"🚗 Les véhicules représentent {percentage:.1f}% de vos émissions. "
                    "Envisagez l'électrification de votre flotte ou le covoiturage."
                )
        
        # Add general recommendations
        if data.pourcentage_local < 50:
            recommendations.append(
                "🌍 Augmentez vos achats locaux pour réduire les émissions de transport."
            )
